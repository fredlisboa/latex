...
\begin{frame}{Quadro Societário Atual}
    \begin{columns}
        \column{0.5\textwidth}
            \textbf{Objetivo:} Dissolução societária com partilha de bens e reconhecimento de passivos.
            \vspace{1cm}
...
\begin{frame}{Encontro de Contas e Conclusão}
    ...
    \begin{tikzpicture}
        % Caixa Dívida
        \node[draw=accentColor, fill=accentColor!10, line width=1pt, rounded corners, align=center, minimum width=4cm, minimum height=1.5cm] (divida) at (0,0) {\textbf{Dívida Aluguel} \\ \reais{1.420.000}};
        
        % Sinal de Menos
        \node[font=\Huge, text=secondaryText] at (2.5,0) {$-$};
        
        % Caixa Bens
        \node[draw=primaryColor, fill=primaryColor!10, line width=1pt, rounded corners, align=center, minimum width=4cm, minimum height=1.5cm] (bens) at (5,0) {\textbf{Bens Entregues} \\ \reais{640.830}};
        
        % Sinal de Igual
        \node[font=\Huge, text=secondaryText] at (7.5,0) {$=$};
        
        % Saldo Restante
        \node[draw=black, fill=lightGray, dashed, align=center, minimum width=4cm, minimum height=1.5cm] (saldo) at (10,0) {\textbf{Saldo Restante} \\ \reais{779.170}};
        
        % Setas e Explicação
        \node[text=successColor, font=\bfseries, align=center] at (10, -1.5) {Valor Perdoado \\ por Kroner};
        \draw[->, thick, successColor] (10, -0.8) -- (10, -1.2);
    \end{tikzpicture}
    \end{center}
